...
use _core::export::{to_bytes, ExportFormat};
...
fn cube_mesh() -> _core::mesher::Mesh {
    mesh(&CubeSdf::new(10.0, 10.0, 10.0), &MeshConfig::centered(8.0, 20))
}
...
fn format_from_path() {
    use std::path::Path;
    assert_eq!(ExportFormat::from_path(Path::new("out.stl")), Some(ExportFormat::Stl));
    assert_eq!(ExportFormat::from_path(Path::new("out.obj")), Some(ExportFormat::Obj));
    assert_eq!(ExportFormat::from_path(Path::new("out.ply")), Some(ExportFormat::Ply));
    assert_eq!(ExportFormat::from_path(Path::new("out.glb")), Some(ExportFormat::Glb));
...
fn empty_mesh_returns_error() {
    ...
    let empty = Mesh { vertices: vec![], triangles: vec![] };
...
fn obj_face_indices_are_1_indexed() {
    ...
    for line in text.lines().filter(|l| l.starts_with('f')) {
        assert!(!line.contains("f 0"), "OBJ face index 0 found — must be 1-based");
    }
}
...
fn ply_binary_size_is_correct() {
    ...
    assert_eq!(binary.len(), expected_binary, "PLY binary body size mismatch");
}
...
fn glb_json_chunk_is_valid_utf8_and_json() {
    ...
    let parsed: serde_json::Value = serde_json::from_str(json_str)
        .expect("GLB JSON chunk is not valid JSON");
...
fn glb_is_4_byte_aligned() {
    ...
    let bin_len  = u32::from_le_bytes(
        bytes[20 + json_len..24 + json_len].try_into().unwrap()
    ) as usize;
