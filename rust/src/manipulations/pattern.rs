...
impl RepeatLinearNode {
    ...
    pub fn row_x(inner: SdfNode, spacing: f32, n: u32) -> Self {
        Self { inner, period: Vec3::new(spacing, 0.0, 0.0), count: [n, 0, 0] }
    }
}
...
mod tests {
    ...
    fn sphere2() -> SdfNode { Box::new(SphereSdf::new(2.0)) }
    ...
    fn mirror_x_reflects_positive_and_negative() {
        ...
        let sdf = MirrorNode { inner: sphere2(), axis: MirrorAxis::X };
    ...
    fn mirror_z_makes_plane_symmetric() {
        let sdf = MirrorNode { inner: sphere2(), axis: MirrorAxis::Z };
    ...
    fn mirror_y_inside_is_inside() {
        let sdf = MirrorNode { inner: sphere2(), axis: MirrorAxis::Y };
    ...
    fn repeat_infinite_x_copies_on_axis() {
        ...
        assert!(sdf.distance(Vec3::new(10.0, 0.0, 0.0)) < 0.0, "copy at x=10 should be inside");
        assert!(sdf.distance(Vec3::new(20.0, 0.0, 0.0)) < 0.0, "copy at x=20 should be inside");
        assert!(sdf.distance(Vec3::new( 0.0, 0.0, 0.0)) < 0.0, "original should be inside");
    }
    ...
    fn repeat_circular_count_1_is_identity() {
        ...
        let sdf = RepeatCircularNode { inner: sphere2(), count: 1 };
        assert_abs_diff_eq!(
