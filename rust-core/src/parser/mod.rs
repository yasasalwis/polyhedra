...
fn parse_err(msg: impl Into<String>) -> PolyhedraError {
    PolyhedraError::GeometryError { message: msg.into() }
}

fn parse_num(pair: Pair<Rule>) -> Result<f32> {
    pair.as_str().parse::<f32>()
...
pub fn parse_str(source: &str) -> Result<PolyhFile> {
    let mut pairs = PolyhParser::parse(Rule::file, source)
        .map_err(|e| {
            let (line, col) = match e.line_col {
                pest::error::LineColLocation::Pos((l, c))       => (l, c),
                pest::error::LineColLocation::Span((l, c), _)   => (l, c),
            };
            PolyhedraError::ParseError { line, col, message: e.variant.message().to_string() }
        })?;

    // `pairs` is a Pairs iterator whose first element is the `file` rule pair.
    // Descend into it to reach the top_level_item children.
    let file_pair = pairs.next().expect("pest always yields at least one pair for file");
...
pub fn parse_file(path: impl AsRef<std::path::Path>) -> Result<PolyhFile> {
    ...
    let source = std::fs::read_to_string(path)
        .map_err(|e| PolyhedraError::FileNotFound {
...
fn build_primitive(pair: Pair<Rule>) -> Result<PrimitiveBlock> {
    ...
    for item in inner {
        if item.as_rule() == Rule::prim_body_item {
            let child = item.into_inner().next().unwrap();
            match child.as_rule() {
                Rule::units_stmt => { units = Some(build_unit(child)?); }
                Rule::prop_stmt  => { props.push(build_prop(child)?); }
                Rule::move_stmt  => { moves.push(build_move(child)?); }
                _ => {}
            }
        }
    }
    Ok(PrimitiveBlock { kind, units, props, moves })
}
...
fn build_export(pair: Pair<Rule>) -> Result<ExportBlock> {
        ...
        if item.as_rule() == Rule::export_body_item {
            ...
            match child.as_rule() {
                Rule::export_format_stmt => {
                    format = Some(
                        child.into_inner().next().unwrap().as_str().to_lowercase()
                    );
                }
                Rule::export_quality_stmt => {
                    quality = Some(match child.into_inner().next().unwrap()
                        .as_str().to_lowercase().as_str()
                    {
                        "low"   => Quality::Low,
                        "high"  => Quality::High,
                        "ultra" => Quality::Ultra,
    ...
    Ok(ExportBlock { format, quality, file })
}
