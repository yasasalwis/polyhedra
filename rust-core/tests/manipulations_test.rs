...
use _core::manipulations::{
    ElongateNode, MirrorAxis, MirrorNode, OffsetNode, RepeatCircularNode, RepeatLinearNode,
    ShellNode,
};
use _core::math::Transform;
use _core::sdf::primitives::{CubeSdf, CylinderSdf, SphereSdf};
use _core::sdf::{Sdf, SdfNode, TransformedSdf, operations::DifferenceNode};
...
fn inset_fillet_preserves_shape() {
    ...
    assert_abs_diff_eq!(
        restore.distance(Vec3::new(5.0, 0.0, 0.0)),
        0.0,
        epsilon = 1e-4
    );
}
...
fn shell_produces_valid_mesh() {
    ...
    assert!(
        m.triangle_count() > 12,
        "shell cube should have inner+outer triangles"
    );
...
fn rounded_cube_with_bore() {
    ...
    let result: SdfNode = Box::new(DifferenceNode {
        a: rounded,
        b: bore,
    });
