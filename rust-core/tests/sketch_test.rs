...
fn rect_symmetry() {
    ...
    assert_abs_diff_eq!(r.distance(p), r.distance(glam::Vec2::new(-3.0, 1.5)), epsilon = 1e-5);
    assert_abs_diff_eq!(r.distance(p), r.distance(glam::Vec2::new(3.0, -1.5)), epsilon = 1e-5);
}
...
fn capsule_length_matches_endpoints() {
    let c = Capsule { a: glam::Vec2::new(-4.0, 0.0), b: glam::Vec2::new(4.0, 0.0), radius: 1.5 };
...
fn offset2d_expands_circle() {
    ...
    assert_abs_diff_eq!(expanded.distance(glam::Vec2::new(5.0, 0.0)), 0.0, epsilon = 1e-5);
}
...
fn extrude_rect_matches_cube() {
    ...
    for p in [Vec3::ZERO, Vec3::new(5.0, 0.0, 0.0), Vec3::new(0.0, 3.0, 0.0), Vec3::new(0.0, 0.0, 2.0)] {
...
fn extrude_hexagon_matches_prism() {
    ...
    for p in [Vec3::ZERO, Vec3::new(5.0, 0.0, 0.0), Vec3::new(0.0, 0.0, 10.0), Vec3::new(8.0, 0.0, 5.0)] {
...
fn revolve_rect_produces_annular_disk() {
    ...
    assert_abs_diff_eq!(revolve.distance(Vec3::new(5.0, 0.0, 0.5)), 0.0, epsilon = 1e-4);
}
...
fn revolve_zero_offset_gives_sphere_metric() {
    ...
    for p in [Vec3::new(r, 0.0, 0.0), Vec3::new(0.0, r, 0.0), Vec3::new(0.0, 0.0, r), Vec3::ZERO] {
...
fn revolve_axis_x() {
    ...
    assert_abs_diff_eq!(revolve.distance(Vec3::new(0.0, 7.0, 0.0)), 0.0, epsilon = 1e-4);
}
...
fn sketch_mesh_is_valid() {
    ...
    let profile = Union2d { a, b: Box::new(Rect::new(3.0, 8.0)) };
